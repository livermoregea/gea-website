...
export default function AdminSlots() {
  ...
  return (
    <div>
      <form onSubmit={addSlot} className="flex flex-wrap items-end gap-3 rounded-sm bg-forest/[0.03] p-5 ring-1 ring-forest/5">
        <div>
          ...
          <input
            ...
            className="mt-2 block rounded-sm border border-forest/15 bg-paper px-3 py-2 text-sm"
          />
        </div>
        <div>
      ...
      <div className="mt-6 space-y-2">
        ...
        {slots.map((s) => (
          <div
            key={s.id}
            className="flex flex-col gap-3 rounded-sm bg-forest/[0.03] px-4 py-3 text-sm ring-1 ring-forest/5 sm:flex-row sm:items-center sm:justify-between"
          >
            <div>
              <p className="font-medium text-graphite">{s.label}</p>
              <p className="font-mono text-xs text-graphite/50">
                {new Date(s.slot_time).toLocaleString()} · {s.is_booked ? "Booked" : "Open"}
              </p>
            </div>
            <button
